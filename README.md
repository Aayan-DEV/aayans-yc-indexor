...
- **Frame budget.** Plain 4-corner bodies, waking only the neighbours of a rising icon, a typing tremor limited to 28 bodies, and `setTransform` drawing. At 1,000 icons: **120 fps** at rest (it was 63), draw **0.72 ms** (it was 1.6), physics 1.8 ms a step at rest and about 5 ms with the whole pile in the air. The enlarged copies of the matches are rendered six per frame rather than all at once, which turned the 89 ms jolt as an answer landed into 37 ms. `window.__floor` shows `stepMs`, `drawMs`, `awake`, `holes`, `gx`, `gy` live.
